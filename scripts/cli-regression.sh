...
# Utility to reset destination and logs
reset_dest() {
    rm -rf "$DEST_DIR"/*
    > $PROJECT_ROOT/regression.log
}

# --- TEST CASE 01: STANDARD SYNC ---
echo "[TC-01] Standard Sync..."
reset_dest
java -jar "$JAR_FILE" sync --source "$SRC_DIR" --destination "$DEST_DIR" --execute --clearState true --undatedFolder "$UNDATED_DIR" >> $PROJECT_ROOT/regression.log 2>&1
...
reset_dest
java -jar "$JAR_FILE" sync --source "$SRC_DIR" --destination "$DEST_DIR" --execute --dryRun true --clearState true --undatedFolder "$UNDATED_DIR" >> $PROJECT_ROOT/regression.log 2>&1
FILE_COUNT=$(find "$DEST_DIR" -type f | wc -l)
...
reset_dest
java -jar "$JAR_FILE" sync --source "$SRC_DIR" --destination "$DEST_DIR" --execute --skipUndated true --clearState true --undatedFolder "$UNDATED_DIR" >> $PROJECT_ROOT/regression.log 2>&1
...
reset_dest
java -jar "$JAR_FILE" sync --source "$SRC_DIR" --destination "$DEST_DIR" --execute --undatedFolder "$UNDATED_DIR" >> $PROJECT_ROOT/regression.log 2>&1
